...
- 2026-03-19: Patch 489: Preview-/Server-Contract minimal gehärtet — Preview-Status wird jetzt zentral ehrlich zwischen `remote_ready`, `fallback_active`, `unavailable`, `loading` und `failed` unterschieden; fehlende/ungültige/unsichere Remote-URLs werden nicht mehr als grüne Server-Preview behandelt, lokaler HTML-Fallback bleibt nutzbar aber sichtbar eingeschränkt, und beide Preview-WebViews laufen mit minimal gehärtetem `mixedContentMode`; Typecheck/Lint/volle Jest-Suite grün.
...
- 2026-03-19: Patch 486: CI-Lite-Header/Modal-Flow jetzt zusätzlich gegen Persistenzdrift gehärtet — Start/Reopen hydriert nur noch frische repo-/branch-/sha-passende CI-Lite-Abschlüsse, bleibt bei fremder/veralteter Persistenz neutral, und das Modal trennt sichtbare Workflow-/Run-Fehler klar von Artifact-/Nachzug-Problemen; gezielte Hydrierungs-, Reopen- und UI-Regressionen ergänzt.
- 2026-03-19: Patch 485: CI-Lite-Header/Modal-Verantwortung sauber getrennt — Header-Tap öffnet nur noch den Status, fachliches Run-Tracking (`trackedRunId`/`isTrackingRun`) läuft unabhängig von `visible` weiter, und gezielte Verhaltenstests fixieren Reopen-/Polling-/Explicit-Start-Verhalten ohne die bestehenden job_id/head_sha-Guards aufzuweichen.
- 2026-03-19: Patch 484: NEW-3 kritisch verifiziert — `new Function()` in `lib/sandpackBuilder.ts` bleibt im lokalen Preview-WebView ein echter Trade-off statt echter Sandbox; als kleiner realer Guardrail liefert der HTML-Fallback jetzt eine restriktive CSP aus, die beliebige Netzwerk-Exfiltration aus diesem Kontext blockiert, ohne den Restpunkt künstlich als „gelöst“ zu verkaufen.
- 2026-03-19: Patch 483: Schritt-8-Restpunkte kritisch verifiziert — im GitHubReposScreen waren G6/G7/G8 im aktuellen Stand nicht als neuer technischer Defekt belegbar; bestätigt blieben nur zwei eng benachbarte Produktpfade: ein Repo-Selection-Race bei asynchroner Default-Branch-Auflösung und ein regressiertes Busy-Wiring im Branch-Manage-Modal. Beide wurden minimal korrigiert und per fokussierten Invariants abgesichert.
- 2026-03-18: Patch 482: Schritt-6-Restpunkte kritisch verifiziert — E8 (kein `main`-Fallback im produktiven Build-Start) und E9 (Build-Start bleibt auf `ProjectContext -> startBuildJob`) sind im aktuellen Stand bereits erfüllt; N7/S7 ließen sich im Repo nicht als aktueller technischer Restpunkt belegen. Einziger bestätigter Rest war E3 als kleiner UX-Mismatch: Fehlt der `x-oauth-scopes`-Header, rendert die Connections-Card keinen irreführenden „unknown (nicht frisch geprüft)“-Badge mehr.
- 2026-03-18: Patch 481: Schritt-5-Restpunkte gezielt verifiziert — S10/E5/E6/E7 im aktuellen Stand ohne neuen technischen Defekt bestätigt; der reale verbleibende B6-Restpunkt wurde minimal geschlossen: Build-Screen und One-Click-Deploy prüfen CI-Lite jetzt auch gegen den zuletzt grünen SHA und den aktuellen Branch-HEAD, sodass die UI nicht mehr grün meldet, wenn `startBuildJob(...)` anschließend wegen SHA-Mismatch blockieren würde.
- 2026-03-18: Patch 480: Schritt-3-Nachprüfung minimal vervollständigt — manuell eingegebene EAS-Project-IDs laufen jetzt vor Save und vor den bestehenden Link-Pfaden durch denselben UUID-Guard, und Chat-Bestätigungstexte zeigen Ownership-/Blocker-Gründe weiterhin explizit statt nur Zähler/Skip-Listen; N6/K8/K9 im aktuellen Stand nicht bestätigt.
- 2026-03-18: Patch 479: Schritt-3-Restpunkte gezielt verifiziert — N6 bereits im aktuellen Connections-/EAS-Flow neutralisiert (kein optimistisches `EAS_OK=true` nach Workflow-Start), N9/K7/K9 im aktuellen Stand nicht als produktiver Defekt bestätigt, und der real verbleibende K8-Restpunkt wurde minimal geschlossen: Validator-Ausfälle sind jetzt im Chat-Fallback für Nutzer sichtbar statt nur im Log.
- 2026-03-18: Patch 478: Schritt-2-Restpunktprüfung konsistent abgeschlossen — NEW-12 bleibt der einzige technisch bestätigte und behobene Punkt im PR-Scope (`x-goog-api-key` Header statt URL-Query-Param), der gezielte Gemini-Regressionstest bleibt bestehen, und README/Patchlog/TODO/Checklog sind jetzt auf denselben Patchstand synchronisiert.
