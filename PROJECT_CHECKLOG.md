## 2026-03-17 — Patch 472: Orchestrator-Hard-Timeout über den gesamten Request-Lauf finalisiert

- `lib/orchestrator/index.ts`: `runOrchestrator(...)` nutzt jetzt ein fixes Gesamtzeit-Budget (`hardDeadlineMs`) statt Timeout-Reset pro Attempt; `remainingMs` begrenzt jeden Provider-Call und beendet den Lauf bei Budgetablauf mit klarem Timeout-Resultat.
- `lib/orchestrator/index.ts`: Timeout-ausgelöste Abbrüche werden deterministisch als Timeout-Fehlertext zurückgegeben; bestehende Abort-/Rotation-Guards bleiben unverändert.
- `lib/__tests__/orchestrator.test.ts`: Timeout-Regression auf expliziten Timeout-Pfad angepasst (`/timeout/i`).
