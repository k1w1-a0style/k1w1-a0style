...
const CustomHeader: React.FC<DrawerHeaderProps> = ({ navigation, options }) => {
  ...
  // --- Export ZIP Handler (Mit Sharing-Fix) ---
  const handleExportZip = async () => {
    ...
    const zipPath = FileSystem.cacheDirectory + 'projekt.zip';
    ...
    try {
        ...
        console.log(`Teile Datei: ${shareableUri}`); // Logge den korrigierten Pfad
        await Sharing.shareAsync(shareableUri, { // Verwende den korrigierten Pfad
  ...
  return (
    // ... (Restliches JSX unverändert) ...
    <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right']}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => navigation.openDrawer()} style={styles.menuButton} disabled={isTriggeringBuild || isExporting}><Ionicons name="menu-outline" size={30} color={theme.palette.text.primary} /></TouchableOpacity>
