...
// Props-Typ für den Screen
type ChatScreenProps = {
  navigation: any;
  route: { params?: { debugCode?: string } };
};

// === ROBUSTERE MessageItem Komponente ===
const MessageItem = memo(({ item }: { item: ChatMessage }) => {
    const messageText = item.text ? String(item.text).trim() : ''; // Stelle sicher, dass es ein String ist und trimme
    
    // Rendere die Blase nur, wenn auch Text vorhanden ist ODER es eine (leere) KI-Nachricht ist
    if (!messageText && item.user._id === 1) {
        // Rendere nichts, wenn der User eine leere Nachricht gesendet hat (sollte nicht passieren)
        return null;
    }
    ...
    return (
        <View style={[ styles.messageBubble, item.user._id === 1 ? styles.userMessage : styles.aiMessage ]}>
             <Text style={item.user._id === 1 ? styles.userMessageText : styles.aiMessageText}>
                {/* Fallback auf '...' wenn KI-Nachricht noch leer ist */}
...
const ChatScreen: React.FC<ChatScreenProps> = ({ navigation, route }) => {
  ...
  const handleSend = useCallback(async (isRetry = false, customPrompt?: string, keyToUse?: string | null) => {
    ...
    // Stelle sicher, dass displayPrompt niemals leer ist
    ...
    if (!isRetry) {
      // Stelle sicher, dass die Text-Eigenschaft immer ein String ist
    ...
    try {
      ...
      // Stelle sicher, dass aiText ein String ist, bevor du speicherst/anzeigst
  ...
  useEffect(() => {
    if (route.params?.debugCode) {
      ...
      setTextInput(`Debug Anfrage...`); // Kürzerer Text
        ...
        <View style={styles.inputContainerOuter}>
            ...
            <View style={styles.inputContainerInner}>
                <TouchableOpacity onPress={handlePickDocument} style={styles.attachButton} disabled={isLoading}><Ionicons name="add-circle-outline" size={28} color={isLoading?theme.palette.text.disabled:theme.palette.primary} /></TouchableOpacity>
                 <TouchableOpacity onPress={handleDebugLastResponse} style={styles.debugButton} disabled={isLoading || messages.filter(m=>m.user._id === 2).length === 0}><Ionicons name="bug-outline" size={24} color={isLoading || messages.filter(m=>m.user._id === 2).length === 0 ? theme.palette.text.disabled : theme.palette.primary} /></TouchableOpacity>
