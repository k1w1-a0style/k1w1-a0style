...
- Patch 472 schließt den verbliebenen Timeout-Hauptrestpunkt nach PR #288: `runOrchestrator` nutzt jetzt eine echte Hard-Deadline über den gesamten Call-Lauf (kein Timeout-Reset pro Retry/Rotation), bleibt abort-konsistent und liefert bei Budgetablauf klaren Timeout-Fehlertext.
- Patch 471 zieht die AI-/Request-Robustheitsreste minimal nach: harter Request-Timeout im Orchestrator (45s), kleiner Rotation-Backoff (350ms) bei 429/Rate-Limit und konservativer Builder-Retry-Backoff (700ms) in `useChatAIFlow`.
