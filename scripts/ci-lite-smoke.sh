...
dispatch_res="$(curl -sS "${SUPABASE_BASE}/functions/v1/github-workflow-dispatch"   -H "content-type: application/json"   -H "x-k1w1-admin-key: ${WORKFLOW_ADMIN}"   -d "${dispatch_payload}")"
...
RUN_ID="$(curl -sS "${SUPABASE_BASE}/functions/v1/github-workflow-runs"   -H "content-type: application/json"   -H "x-k1w1-admin-key: ${WORKFLOW_ADMIN}"   -d "${run_payload}" | jq -r '.data.workflow_runs[0].id // .workflow_runs[0].id // .runs[0].id // empty')"
...
conclusion="unknown"
for i in {1..40}; do
  meta="$(curl -sS "${SUPABASE_BASE}/functions/v1/github-workflow-logs"     -H "content-type: application/json"     -H "x-k1w1-admin-key: ${WORKFLOW_ADMIN}"     -d "$(jq -n --arg repo "$REPO" --argjson runId "$RUN_ID" '{githubRepo:$repo, runId:$runId, summary:true}')" )"
...
summary="$(curl -sS "${SUPABASE_BASE}/functions/v1/github-workflow-logs"   -H "content-type: application/json"   -H "x-k1w1-admin-key: ${WORKFLOW_ADMIN}"   -d "$(jq -n --arg repo "$REPO" --argjson runId "$RUN_ID" '{githubRepo:$repo, runId:$runId, summary:true}')" )"
