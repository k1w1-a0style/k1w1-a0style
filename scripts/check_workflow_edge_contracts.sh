...
DEPLOY_SUPABASE_WF=".github/workflows/deploy-supabase-functions.yml"
...
require_pattern "$TRIGGER_ROUTE_CORE" 'require(ScopedEdgeAuth|OwnerOrJwtAuth)\(req,\s*\{'
require_pattern "$CHECK_ROUTE_CORE" 'requireScopedEdgeAuth\(req,\s*\{'
require_pattern "$ARTIFACT_EDGE" 'requireScopedEdgeAuth\(req,\s*\{'
require_pattern "$RUNS_EDGE" 'require(ScopedEdgeAuth|OwnerOrJwtAuth)\(req,\s*\{'
require_pattern "$LOGS_EDGE" 'require(ScopedEdgeAuth|OwnerOrJwtAuth)\(req,\s*\{'
require_pattern "$KEYSTORE_EDGE" 'require(ScopedEdgeAuth|OwnerOrJwtAuth)\(req,\s*\{'
require_pattern "$KEYSTORE_GENERATE_EDGE" 'requireOwnerOrJwtAuth\(req,\s*\{'
require_pattern "$KEYSTORE_STATUS_EDGE" 'requireOwnerOrJwtAuth\(req,\s*\{'
require_pattern "$DISPATCH_EDGE" 'require(ScopedEdgeAuth|OwnerOrJwtAuth)\(req,\s*\{'

require_fixed "$TRIGGER_ROUTE_CORE" 'adminSecretEnv: "K1W1_EDGE_WORKFLOW_ADMIN_KEY"'
require_fixed "$TRIGGER_ROUTE_CORE" 'requireOwnerOrJwtAuth(req, {'
require_fixed "$CHECK_ROUTE_CORE" 'adminSecretEnv: "K1W1_EDGE_WORKFLOW_ADMIN_KEY"'
require_fixed "$CHECK_ROUTE_CORE" 'requireWorkflowOperatorJwtRoleWithVerifiedActor(req, "check-eas-build")'
...
require_fixed "$RUNS_EDGE" 'requireOwnerOrJwtAuth(req, {'
require_fixed "$LOGS_EDGE" 'adminSecretEnv: "K1W1_EDGE_WORKFLOW_ADMIN_KEY"'
require_fixed "$LOGS_EDGE" 'requireOwnerOrJwtAuth(req, {'
require_fixed "$DISPATCH_EDGE" 'adminSecretEnv: "K1W1_EDGE_WORKFLOW_ADMIN_KEY"'
require_fixed "$DISPATCH_EDGE" 'requireOwnerOrJwtAuth(req, {'
...
require_fixed "$KEYSTORE_EDGE" 'requireOwnerOrJwtAuth(req, {'
require_fixed "$KEYSTORE_GENERATE_EDGE" 'requireOwnerOrJwtAuth(req, {'
require_fixed "$KEYSTORE_STATUS_EDGE" 'requireOwnerOrJwtAuth(req, {'
require_fixed "$KEYSTORE_GENERATE_EDGE" 'adminSecretEnv: "K1W1_EDGE_ANDROID_KEYSTORE_EXPORT_ADMIN_KEY"'
require_fixed "$KEYSTORE_GENERATE_EDGE" 'requireJwtRoleWithVerifiedActor: requirePrivilegedOperatorJwtRoleWithVerifiedActor'
...
require_fixed "$KEYSTORE_STATUS_EDGE" 'requireJwtRoleWithVerifiedActor: requirePrivilegedOperatorJwtRoleWithVerifiedActor'
...
require_fixed "$SAVE_PREVIEW_EDGE" 'requireOwnerOrJwtAuth(req, {'
forbid_fixed "$SAVE_PREVIEW_EDGE" 'requireScopedEdgeAuth(req, {'
require_fixed "$SAVE_PREVIEW_EDGE" 'guard: await requireVerifiedJwt(request, scope)'
...
require_fixed "$WIZARD_HELPERS" 'headers.Authorization = `Bearer ${bearerToken}`;'
...
require_fixed "$K1W1_HANDLER_EDGE" 'requireAiOperatorJwtRoleWithVerifiedActor(req, "k1w1-handler")'
...
require_fixed "$DEPLOY_SUPABASE_WF" '[ "$name" != "_shared" ] && [ "$name" != "test" ]'
require_fixed "$DEPLOY_SUPABASE_WF" 'Refusing to deploy legacy disabled function: test'
