...
describe("recoverable multi-store persistence invariants", () => {
  ...
  it("keeps Connections recovery scope journal/snapshot-driven instead of current repo closure", () => {
    ...
    expect(src).toContain("const repoScopeAtSaveStart = effectiveRepo;");
    expect(src).toContain("readCurrentSnapshot(repoScopeAtSaveStart)");
    expect(src).toContain("persistSelectedEasProjectId(plan.easProjectId, repoScopeAtSaveStart);");
