...
describe("patch549 keystore export JWT/RBAC hardening invariants", () => {
  ...
  it("keeps deny-by-default JWT role checks in shared auth and the route entrypoint", () => {
    ...
    expect(sharedAuth).toContain("WorkflowOperatorJwtGuardWithActorResult");
    ...
    expect(route).toContain("const auth = await requireOwnerOrJwtAuth(req, {");
    expect(route).toContain("const actor = auth.actor ?? \"scoped_admin\";");
    expect(route).toContain("requireJwtRoleWithVerifiedActor: requireServiceRoleJwtWithVerifiedActor");
