...
describe('Orchestrator', () => {
  ...
  describe('runOrchestrator', () => {
    ...
    it('faellt bei serverseitig fehlendem Provider automatisch auf einen verfuegbaren Provider zurueck', async () => {
      const testMessages: LlmMessage[] = [{ role: 'user', content: 'hi' }];
      ...
      const result = await runOrchestrator('gemini', 'gemini-3-flash', 'speed', testMessages);

      expect(result.ok).toBe(true);
      expect(result.provider).toBe('openai');
      expect(result.model).toBe('gpt-4o');
      expect(result.errors?.[0]).toContain('Provider-Fallback aktiv');
      expect(invokeMock).toHaveBeenCalledTimes(2);
    });

    it('faellt bei unbekanntem Modell zuerst auf das Provider-Default-Modell zurueck', async () => {
      const testMessages: LlmMessage[] = [{ role: 'user', content: 'hi' }];
      ...
      invokeMock
        .mockResolvedValueOnce({
          data: {
            ...
            error: 'Das Modell "gpt-5.4-pro" ist bei Openai nicht verfuegbar oder wird dort nicht unterstuetzt.',
            provider: 'openai',
            model: 'gpt-5.4-pro',
            status: 404,
          },
        ...
        .mockResolvedValueOnce({
          data: {
            ...
            provider: 'openai',
            model: 'gpt-5.4',
            content: 'default fallback ok',
          },
          error: null,
        });

      const result = await runOrchestrator('openai', 'gpt-5.4-pro', 'quality', testMessages);

      expect(result.ok).toBe(true);
      expect(result.provider).toBe('openai');
      expect(result.model).toBe('gpt-5.4');
      expect(result.errors?.[0]).toContain('Modell-Fallback aktiv');
      expect(invokeMock).toHaveBeenCalledTimes(2);
    });
