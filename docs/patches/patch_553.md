...
- `supabase/functions/github-workflow-dispatch/index.ts`
  ...
  - Erlaubte Claims explizit: `service_role`, `authenticated`, `anon`.
...
- `components/CiLiteHeaderButton/hooks/useCiLiteWorkflow.ts`
  - Dispatch-Caller sendet jetzt zusaetzlich `Authorization: Bearer <SUPABASE_ANON_KEY>` (neben `x-k1w1-admin-key`).
  - Klare Fehlermeldung, wenn `SUPABASE_ANON_KEY` lokal fehlt.
