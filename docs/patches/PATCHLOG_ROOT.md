...
## Recent (kompakt)
- Patch 463: SettingsScreen/AIContext-Restpunkte konservativ geschlossen — Quality-Mode setzt nun die effektiven Chat-/Agent-Modelle mit (kein kosmetischer Toggle), Retention-Limit ist in Privacy sinnvoll editierbar/persistierbar, Gemini-Key-Prefix-Validation (`AIza`) ergänzt, unnötige `any`-Hotspots im Settings-Hook/Provider-Status entfernt und gezielte Tests für Quality-Mode-/Settings-Helper ergänzt.
