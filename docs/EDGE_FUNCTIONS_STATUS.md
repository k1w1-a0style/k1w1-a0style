...
- Patch 757 dokumentiert den verbleibenden Preview-CSP-Tradeoff explizit und eng: `preview_page` darf fuer Sandpack standardmaessig weiter `unsafe-eval` + `https://esm.sh` nutzen, ist aber bewusst hart abschaltbar (`TEST_STRICT_CSP=true` oder `PREVIEW_STRICT_CSP=true` oder `PREVIEW_ALLOW_UNSAFE_EVAL=false`; CDN-Import optional via `PREVIEW_ALLOW_ESM_SH_CDN=false`). Dieser Pfad bleibt ein bewusstes Sandbox-Tradeoff und kein voll risikofreier CSP-Mode.
