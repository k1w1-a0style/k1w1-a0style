...
| `save_preview` / `preview_page` | Persistiert und rendert Browser-Previews | `previewUrl`, `expiresAt` (save) / HTML-Response + sichere Fehlerfälle (page) · Auth: `save_preview` nutzt jetzt **verifiziertes Supabase-Login-JWT** (`verify_jwt=true`, `requireVerifiedJwt(...)`, kein lokaler Legacy-Admin-Key mehr), `preview_page` bleibt public secret-link |
| `k1w1-handler` | KI-Provider-Proxy für produktive Chat-Calls | `ok`, `provider`, `model`, `content`, `raw` bzw. `ok:false,error` · Auth: **JWT + Claim** (`verify_jwt=true`, `role in [service_role, build_admin]`, `Authorization: Bearer <jwt>`, kein lokaler Legacy-Admin-Key mehr) |
...
- Operativer Hinweis (Live-Drift): `scripts/check_edge_live_contracts.sh` validiert nur Laufzeitverhalten. Ein manuell umgestelltes Supabase-Flag `verify_jwt=false` kann dadurch unentdeckt bleiben; fuer `save_preview` und `k1w1-handler` bleibt ein expliziter Live-Flag-Abgleich vor Release verpflichtend.
