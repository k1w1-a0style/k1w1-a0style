...
- [x] **Deep-Scan-SoT-/Header-Drift korrigiert und Debt-Lage neu inventarisiert:** Kern-Standheader (`INDEX`/`TESTING_GUIDE`/`FRESH_CHECKOUT_GREEN_PATH`) auf denselben Patchstand gezogen; `docs/04-risk-hotspots.md` um aktuelle Zaehler (`as any`: 331 gesamt, 150 codefokussiert; `: any`: 170 gesamt, 152 codefokussiert) inkl. Runtime-vs-Test-Einordnung erweitert; `docs/reviews/deep-scan-review-2026-03-30.md` um Addendum mit bestaetigten Restpunkten/Non-Issues nachgeschaerft (Patch 638).
...
- [x] **Test-Determinismus fuer CI-Freshness-Guard nachgezogen:** `__tests__/buildReadinessGate.ciLiteFreshness.test.ts` nutzt eine feste Uhrbasis (`FIXED_NOW`) plus `Date.now`-Spy/Restore statt verstreuter Echtzeitaufrufe; Deep-Scan-Review enthaelt dazu den finalen Test-Audit-Block mit weiteren priorisierten Verbesserungsoptionen (Patch 638).
- [x] **PR-506 Doku-/Review-SoT konsolidiert:** Kern-MD-Standheader (`INDEX`/`TESTING_GUIDE`/`FRESH_CHECKOUT_GREEN_PATH`) harmonisiert, Deep-Scan-Hauptteil auf aktuellen TODO-Status korrigiert (keine ueberholten „offen“-Aussagen mehr fuer geschlossene Punkte), und `AGENTS.md` um verbindlichen Kern-MD-/SoT-Abgleich plus Abschlussbericht-Pflichtangaben nachgeschaerft (Patch 638).
